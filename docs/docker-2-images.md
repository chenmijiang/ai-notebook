...
**Docker Hub 免费账户限制**（2026 年）：

| 功能         | 免费账户      | Pro 账户 |
| ------------ | ------------- | -------- |
| 公共仓库     | 无限          | 无限     |
| 私有仓库     | 1 个          | 无限     |
| 拉取速率限制 | 100 次/6 小时 | 无限     |
| 并行构建     | 1 个          | 5 个     |

> **提示**：匿名拉取（未登录）有更严格的速率限制（100 次/6 小时，按 IP 计算）。建议始终 `docker login` 后操作。
...
> **注意**：生产环境应始终使用摘要（Digest）而非标签来引用镜像，防止标签被覆盖指向恶意镜像。
