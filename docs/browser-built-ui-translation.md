...
浏览器接收这些信息，拉取数据，再根据“你是谁”“你想完成什么”，借助 LLM 推理出最合适的呈现方式，生成一个贴合用户需求的界面。

## 这件事的重要性，可能比听起来更大

我之前在 Xero 做应用商店和集成平台时，始终被一个问题困扰：每个第三方集成都有自己的一套 UI 约定。用户每接入一个新应用，就得重新学习一套界面。如果 UI 是浏览器基于一组共享偏好自动生成的，这个问题基本就消失了。
...
API 设计会一下子变得重要得多。你不能再随手拼几个 REST 端点就算完事。你的 manifest 必须具备语义，它描述的不只是“数据长什么样”，还要说明“这些数据意味着什么”。服务之间的数据契约会更重要，版本管理也会更重要。
...
我做了一个 [adaptive browser](https://github.com/jonnonz1/adaptive-browser) 作为 PoC（概念验证），来测试这套想法。它使用 Claude，根据 GitHub manifest 和 YAML 形式的用户偏好生成 UI。现在它还比较初步，但我认为方向是对的。
