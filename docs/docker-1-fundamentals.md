...
> **注意**：Docker 容器依赖 Linux 内核特性运行。在 macOS 和 Windows 上，Docker Desktop 通过一个轻量级 Linux 虚拟机来运行容器，而非直接使用宿主内核。这一点在后续的网络（[第 5 篇](docker-5-networking.md)）和存储（[第 6 篇](docker-6-storage.md)）章节中会有具体影响。
...
│   容器可写层             │  ← 容器运行时自动添加
├──────────────────────────┤
│   npm install 依赖层     │  ← 只读（来自镜像）
├──────────────────────────┤
│   Node.js 运行时层       │  ← 只读（来自镜像）
├──────────────────────────┤
│   Debian 基础系统层      │  ← 只读（来自镜像）
└──────────────────────────┘
```
...
| 7    | 容器退出     | 程序执行完毕，容器进入 Exited 状态（`docker ps -a` 可查看） |
