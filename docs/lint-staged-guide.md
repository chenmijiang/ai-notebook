...
> **注意**：返回函数（不带参数）时，lint-staged 不会将文件列表追加到命令。
...
| 选项                 | 说明                     | 默认值 |
| -------------------- | ------------------------ | ------ |
| `--concurrent`       | 并行运行不同 glob 的任务 | `true` |
| `--concurrent=false` | 顺序执行所有任务         | -      |
| `--concurrent=2`     | 最多 2 个任务并行        | -      |
...
```javascript
// ✅ 好的实践：让 ESLint 一次处理所有文件
{
  "*.js": "eslint --fix"
}

// ❌ 坏的实践：每个文件单独启动 ESLint 进程
{
