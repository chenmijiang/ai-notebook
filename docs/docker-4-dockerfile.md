...
> **进阶**：需要极致可复现性时，可在标签后追加 digest 锁定到具体镜像内容：`FROM node:22-slim@sha256:abcd1234...`。digest 可通过 `docker inspect --format='{{index .RepoDigests 0}}' node:22-slim` 获取。注意锁定 digest 后不会自动获得安全更新，需要定期手动更新。
...
> **注意**：上面的 `groupadd`/`useradd` 适用于 Debian 系基础镜像（如 `node:22-slim`）。Alpine 镜像使用 BusyBox 工具链，应改用 `addgroup -S appuser && adduser -S -G appuser appuser`。
